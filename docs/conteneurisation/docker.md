...
Pour retirer un conteneur ou une image précise, `docker rm <conteneur>` et `docker rmi <image>` suffisent — voir les sections ci-dessus. Pour désinstaller Docker entièrement de l'hôte :
...
!!! danger "Irréversible"
    Cette séquence supprime tous les conteneurs, images et volumes de l'hôte, y compris ceux sans lien avec cette fiche. La réserver à une désinstallation complète décidée délibérément, jamais à un retour arrière partiel.

Si seul le réseau `docker0` a été personnalisé (`/etc/docker/daemon.json`), la marche à suivre plus ciblée est déjà détaillée dans l'encart *Revenir en arrière* de la section [Réseau par défaut](#reseau-par-defaut).
